...
public class LogManagerTests {
    ...
    public void TestSimple() {
        ...
        byte[] read_r = lm.readLogRecord(0, r_len);//返回偏移为position，长度为len
        Record rec_r = Record.deserialize(read_r);//反序列化
        ...
        assert(lm.getLogEndOffset() == r_len + r2_len);//返回最后一次写入后的偏移
